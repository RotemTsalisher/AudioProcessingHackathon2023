...
[hydrophone1,Fs]=audioread('hydrophone1.wav');
[hydrophone2,Fs]=audioread('hydrophone2.wav');
[hydrophone3,Fs]=audioread('hydrophone3.wav');
...
shift_2=fix(5.40*Fs);
shift_3=fix(7.58*Fs);
[hydrophone1,Fs]=audioread('hydrophone1.wav',[1+shift_1 shift_1+n_samples]);
[hydrophone2,Fs]=audioread('hydrophone2.wav',[1+shift_2 shift_2+n_samples]);
[hydrophone3,Fs]=audioread('hydrophone3.wav',[1+shift_3 shift_3+n_samples]);
...
input=audioread('hydrophone3.wav',[465000 466650]);
...
standard_xcorr_tk = rotem_gcc(signal_1_tk, signal_2_tk, 'cc');
[val,ind]=max(standard_xcorr_tk);
TDOA_standard_xcorr_tk = ind-M; % in samples
disp(['ROTEM FUNCTION: results: TDOA_standard_xcorr_tk: ' num2str(TDOA_standard_xcorr_tk)]);
...
phase_transform_tk = rotem_gcc(teager_kaiser(signal_1), teager_kaiser(signal_2),  'phat');
[val,ind]=max(phase_transform_tk);
TDOA_phase_transform_tk = ind-M;
disp(['ROTEM FUNCTION: results: TDOA_phase_transform_tk: ' num2str(TDOA_phase_transform_tk)]);
...
signal_1=audioread('hydrophone2.wav',[460000+shift_2 ...
    460000+shift_2+2000]);
signal_2=audioread('hydrophone3.wav',[460000+shift_3 ...
    460000+shift_3+2000]);
...
phase_transform_tk  = rotem_gcc(signal_1_tk, signal_2_tk,  'phat');
[val,ind]=max(phase_transform_tk );

%AS MENTIONED ABOVE: % We now apply GCC to real hydrophone signals (notice we . Clicks are separated by
% about 400 samples.
...
TDOA_12 = rotem_TDOA_LMS(signal_1_tk,signal_2_tk,600,0.01);
...
[TDOA_23,peak] = rotem_TDOA_LMS(signal_2_tk,signal_3_tk,600,0.01);
